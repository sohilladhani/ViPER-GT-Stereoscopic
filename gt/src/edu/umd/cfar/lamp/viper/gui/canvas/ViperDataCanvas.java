...
import viper.api.impl.ViperParser;
...
import edu.umd.cfar.lamp.chronicle.extras.ScrollWheelPanEventHandler;
import edu.umd.cfar.lamp.chronicle.extras.ScrollWheelZoomHandler;
...
public class ViperDataCanvas extends PCanvas {
	...
	public ViperDataCanvas() throws IOException {
		...
		this.setPreferredSize(new Dimension(560, 240)); // added by Sohil - Nov
														// 3, 2016
		...
		ScrollWheelZoomHandler zHandler = new ScrollWheelZoomHandler();
		...
		this.getCamera().addPropertyChangeListener(
				PCamera.PROPERTY_VIEW_TRANSFORM, listenForZooms);
	}
	...
	public static void main(String[] args) throws RDFException,
		...
		try {
			...
			String firstFile = ((Sourcefile) mediator.getViperData()
	...
	public void setViperView(ViperDataPLayer view) {
		if (viperView != null) {
			removeInputEventListener(viperView.editManager);
		}
		...
		if (viperView != null) {
			...
			// Set up keyboard events to go to right place
			viperView.connectToCanvas();

			addInputEventListener(viperView.editManager);
		}
	}
	...
	private ActionListener setZoomLevelActionListener = new ActionListener() {
		/**
		 * Sets the zoom level to the amount specified in the event's action
		 * command property.
		 */
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				if ("first".equals(zoomStr)) {
					if (!cameraTransformHistory.get(
							cameraTransformHistory.size() - 1).equals(currZoom)) {
					...
					// zoom command. This involves will likely involve pushing a
					// new
					...
					if ("actual".equals(zoomStr)) {
						...
						// XXX: zoom to fit should take into acount scroll bars,
						// if they exist
						// ^^^^ also, remember to be careful that on macs, the
						...
						Point2D center = currZoom.inverseTransform(cam
								.getBoundsReference().getCenter2D(), null);
						double Y = cam.getBoundsReference().getCenterY();
						if (zoomStr.startsWith("x")) {
							zoomFactor *= Double.parseDouble(zoomStr
									.substring(1));
						} else if (zoomStr.endsWith("%")) {
							zoomFactor = Double.parseDouble(zoomStr.substring(
									0, zoomStr.length() - 1));
							zoomFactor /= 100.0;
							zoomFactor = zoomFactor / newZoom.getScale();
						} else {
							logger.log(Level.SEVERE,
									"Unable to zoom by specified amount: "
											+ zoomStr);
							return;
						}
						newZoom = new PAffineTransform();
						newZoom.scaleAboutPoint(zoomFactor, center.getX(),
								center.getY());
					}
					if (newZoom != null) {
						...
						if (cameraTransformCursor < cameraTransformHistory
								.size()) {
							if (!cameraTransformHistory.get(
									cameraTransformCursor - 1).equals(currZoom)) {
								cameraTransformHistory.add(
										cameraTransformCursor, currZoom);
								cameraTransformCursor++;
							}
							if (!cameraTransformHistory.get(
									cameraTransformCursor - 1).equals(newZoom)) {
								cameraTransformHistory.subList(
										cameraTransformCursor,
										cameraTransformHistory.size()).clear();
								newCursorLocation = cameraTransformHistory
										.size();
								cameraTransformHistory.add(newZoom);
							}
						} else {
							if (!cameraTransformHistory.getLast().equals(
									currZoom)) {
				...
				if (0 <= newCursorLocation) {
					AffineTransform at = (PAffineTransform) cameraTransformHistory
							.get(newCursorLocation);
				...
				logger.log(Level.SEVERE, "Unable to zoom by specified amount: "
						+ zoomStr, iax);
			} catch (NoninvertibleTransformException ntx) {
				logger.log(Level.SEVERE,
						"Unexpected error while setting zoom.", ntx);
			}
		}
	...
	private PropertyChangeListener listenForZooms = new PropertyChangeListener() {
		public void propertyChange(PropertyChangeEvent evt) {
			...
			if (currTime - 1024 > lastSetTransformTime
					&& lastSetTransformValue != null) {
				if (!cameraTransformHistory.get(cameraTransformCursor - 1)
						.equals(lastSetTransformValue)) {
					cameraTransformHistory.add(cameraTransformCursor,
	 ...
	 * <dl>
	 * <dt>first</dt>
	 * <dd>Goes to the original zoom; like <code>actual</code>, but goes to
	 * start of list instead of adding to it.</dd>
	 * <dt>previous</dt>
	 ...
	 * <dt>fit</dt>
	 * <dd>Fits the image to the window, centering it (or should it be in the
	 * upper-left corner?)</dd>
	 * <dt>x<i>number</i></dt>
	 * <dd>Multiplies the zoom by the specified amount, keeping the center point
	 * fixed</dd>
	 * <dt><i>number</i>%</dt>
	 * <dd>Sets the zoom to the specified amount, keeping the center point
	 * fixed.</dd>
	 * </dl>
	 * 
