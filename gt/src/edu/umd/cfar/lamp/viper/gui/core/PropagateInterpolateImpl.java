...
public class PropagateInterpolateImpl implements PropagateInterpolateModule {
	...
	private void helpInterpolateAttribute(Attribute a, Instant start, Span s,
		...
		if (ac.isDynamic() && (size > 0)) {
			if (magic == null) {
				if (size == 1) {
					...
					helpPropagateAttribute(a, begin, firstHalf);
					helpPropagateAttribute(a, end, secondHalf);
				}
			} else {
				Object sVal = a.getAttrValueAtInstant(begin);
				Object eVal = a.getAttrValueAtInstant(end);
				if (sVal == null || eVal == null) {
					helpInterpolateAttribute(a, start, s, null);
				} else {
						...
						if (wrt != null && (sVal instanceof Moveable)
							...
							do {
								...
								nval = nval.shift(sWrt.x.intValue(), sWrt.y
										.intValue());
								a.setAttrValueAtSpan(nval, ns);
							} while (changeIter.hasNext());
						} else {
							a.setAttrValueAtSpan(dval.getValue(), ns);
						}
					}
				}
			}
					...
					if (mediator.getHiders().getDescriptorVisibility(d) > NodeVisibilityManager.RANGE_LOCKED) {
						...
						if (mediator.getChronicleSelectionModel() != null) {
							if (mediator.getChronicleSelectionModel().getSelectedTime() != null) {
								selectedRange = mediator.getChronicleSelectionModel().getSelectedTime().iterator(s);
							}
						}
						if (!r.contains(s)) {
							while(selectedRange.hasNext()) {
