...
import edu.umd.cfar.lamp.viper.util.SwingWorker;
...
public class ViperViewMediator {
	...
	private PropagateInterpolateModule propagator = null;
	...
	public ViperViewMediator() {
		...
		getPrimarySelection().addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent event) {
				...
				// anyway.
				Sourcefile currFile = getCurrFile();
				Sourcefile addedFile = e.getAddedNodes().getFirstSourcefile();
				if ((currFile == null) ? (addedFile != null) : (!currFile
						.equals(addedFile))) {
					// this is an old event; selection is now something else
					return;
				}
				Sourcefile removedFile = e.getRemovedNodes()
						.getFirstSourcefile();
	...
	private class Loader extends SwingWorker {
		...
		public Object construct() {
			...
			return DataPlayer.createDataPlayer(local == null ? null : new File(local), getPrefs());
		}
	...
	private boolean saveToFile(File f) throws IOException {
		...
		PrintWriter pw = new PrintWriter(new FileWriter(tmpSave));
		try {
		...
		public void actionPerformed(ActionEvent event) {
			if (getCurrFile() != null) {
				try {
					...
					ImportExport.exportSourcefileAsGTF(getCurrFile());
				} catch (IOException e) {
	...
	private ActionListener importGTFFileListener = new ActionListener() {
		public void actionPerformed(ActionEvent event) {
			...
			ImportExport.importDataFromGTF(getCurrFile());
		}
	};
	...
	private ActionListener importConfigListener = new ActionListener() {
		...
		public void actionPerformed(ActionEvent event) {
			try {
				...
				ImportExport.importConfig(getViperData());
			} catch (IOException e) {
				logger.log(Level.SEVERE,
