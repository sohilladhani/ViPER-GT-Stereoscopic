...
public class ViperControls {
	...
	private class Clicker implements ActionListener {
		private Instant lastTicksFrame;
			...
			if (!paused && null != mediator) {
				...
				if (isRealtimePlayback()) {
					...
					if (lastTickTime > 0) {
						...
						TemporalRange tr = getPlaybackRange();
						Rational start = new Rational (((Instant) tr.getExtrema().getStart()).longValue());
						Rational end = new Rational (((Instant) ((Instant) tr.getExtrema().getEnd()).previous()).longValue());
						if (temp.lessThan(start)) {
							temp = start;
						} else if (temp.greaterThan(end)) {
							temp = end;
						}
						long oldTick = lastTicksFrame.longValue();
						long newTick = temp.longValue();
						if (oldTick != newTick) {
							smartGo(lastTicksFrame.go(newTick - oldTick));
						}
