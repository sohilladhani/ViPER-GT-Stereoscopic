...
abstract class DataPlayerHelper extends DataPlayer {
	private final List lastFrameCache = new ArrayList();
	private int cacheSize = 1;

	/** keeps to keep weak references to used images */
	private final WeakHashMap weakCache = new WeakHashMap();
	...
	private final HashMap strongCache = new HashMap();
	...
	private final FrameWorkerQueue frameLoader = new FrameWorkerQueue();
			...
			Runnable runLoop = new Runnable() {
				public void run() {
					try {
						...
						logger.log(Level.FINE, "Interrupted while decoding a frame", ie);
					} catch (Exception e) {
						logger.log(Level.WARNING, "Error while decoding a frame", e);
					}
				}
			};
