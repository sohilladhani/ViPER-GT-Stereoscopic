...
public class TextlineCanvasCreator extends CanvasCreator {
	...
	public void mouseReleased(PInputEvent e) {
		...
		if(currentState == TEXTLINE_ALL_FINISHED) {
			...
			/* NOTE: no longer popping up text box
			// prompt for text input
			if (tlm != null) {
				text = tlm.getText();
			}
			// validate the input: no empty or whitespace-only strings
			while(text == null || text.equals("") || text.matches("^\\s+$")) {
				text = JOptionPane.showInputDialog(null, "Please enter text:", "Textline", JOptionPane.PLAIN_MESSAGE);
			}*/
			
			//System.out.println("Handing control over to TextlineCanvasEditor"); // DEBUG
			getAssistant().switchListener() ; // switch from creator to editor
			tlm.getTextPointer().setAttrValue(text);
			tlm = new TextlineModel( (int) oboxPts[ 0 ].getX(),
					   (int) oboxPts[ 0 ].getY(),
					   (int) oboxWidth, (int) oboxHeight, 
					   (int) Math.toDegrees(angle), text);
