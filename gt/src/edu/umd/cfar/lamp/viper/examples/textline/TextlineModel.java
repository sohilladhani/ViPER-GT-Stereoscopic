...
public class TextlineModel implements BoxInformation, HasCentroid, Moveable {
	...
	public void setText(String text) {
		...
		for(int i = getMaxBoundaries(); i < wordOffsets.size(); /* no i++ on purpose*/ ) wordOffsets.remove(i);
	}
	...
	public String[] getOccludedWords() {
		...
		for(int i = 0; i < letters.length; i++) {
			inrange = currOcc.rangeContains(i);
			...
			if(inrange == 1) {
				...
				inrange = currOcc.rangeContains(i); // IMPORTANT: re-check
			}
			// if inside the range, then we need to occlude BUT NOT if it's a space
			if(inrange == 0 && letters[i] != ' ') letters[i] = '-';
	...
	public Object clone() {
		...
		tm.setText(getText()); // TODO: remove this later
		return tm;
	}
