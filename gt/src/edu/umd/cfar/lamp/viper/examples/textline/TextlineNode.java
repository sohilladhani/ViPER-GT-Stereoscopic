...
public class TextlineNode extends PBoxNode implements Attributable {
	...
	public void setPath(TextlineModel box) {
		...
		text.setText(box.getOccludedWords()[0]); // display only the first word
	...
	public void updateOffsets() {
		...
		String[] words = changedCopy.getOccludedWords();
		offscreenCountL = 0;
		offscreenCountR = 0;
		...
		for(int i = 0; i < wo.size(); i++) {
			...
			text.setText(changedCopy.getOccludedWords()[offscreenCountL]); // display only the first word not offscren on the left
	...
	public Object getUpdatedAttribute() {
		...
		TextlineModel textlineModel = new TextlineModel((int) oboxPts[0].getX(),
				(int) oboxPts[0].getY(), (int) oboxWidth, (int) oboxHeight,
				degrees, origBox.getText(), origBox.getOcclusions(), origBox.getWordOffsets());
	...
	public void boldNorthHandle() {
		...
		if (!northHandle.getChildrenReference().contains(upLabel)) {
			((PTextLabel) upLabel).setInset(0);
			((PTextLabel) upLabel).setRadius(8);
